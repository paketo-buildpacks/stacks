...
func testTiny(t *testing.T, when spec.G, it spec.S) {
	...
	it("can get the list of packages", func() {
		buildPackages, err := tiny.GetBuildPackagesList(buildImageTag)
		Expect(err).NotTo(HaveOccurred())
		...
		runPackages, err := tiny.GetRunPackagesList(runImageTag)
		Expect(err).NotTo(HaveOccurred())
	...
	it("can get the package metadata", func() {
		buildPackageMetadata, err := tiny.GetBuildPackageMetadata(buildImageTag)
		Expect(err).NotTo(HaveOccurred())
		...
		runPackageMetadata, err := tiny.GetRunPackageMetadata(runImageTag)
		Expect(err).NotTo(HaveOccurred())
